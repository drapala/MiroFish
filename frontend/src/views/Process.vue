      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">Aguardando geração de ontologia</p>
            <p class="waiting-hint">A construção iniciará automaticamente após a geração</p>
          </div>
          
          <!-- Em construção sem dados -->
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">Grafo em construção</p>
            <p class="waiting-hint">Dados em breve...</p>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Descrição da Interface</div>
                <div class="detail-content">
                  Após upload, o LLM analisa o conteúdo e gera a estrutura de ontologia (tipos de entidade + tipos de relação)
                </div>
              </div>
              
              <!-- Progresso de ontologia -->
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">Aguardando geração de ontologia...</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Descrição da Interface</div>
                <div class="detail-content">
                  Com base na ontologia, os documentos são segmentados e enviados ao Zep API para construção do grafo
                </div>
              </div>
              
              <!-- Aguardando ontologia -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">Aguardando conclusão da geração de ontologia...</div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">Próxima etapa</div>
              </div>
...
// ID do projeto atual (pode mudar de 'new' para real)
...
const ontologyProgress = ref(null) // Progresso de ontologia
const currentPhase = ref(-1) // -1: Enviando, 0: Ontologia, 1: Grafo, 2: Concluído
const selectedItem = ref(null) // Nó/aresta selecionado
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 1) return 'Grafo em construção'
...
const goToNextStep = () => {
  // TODO: Entrar na Config. do Ambiente
  alert('Config. do ambiente em desenvolvimento...')
}
...
// Novo projeto - API ontology/generate
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'Nenhum arquivo, retorne à página inicial'
    loading.value = false
    return
  }
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // Fase de ontologia
    ...
    if (response.success) {
      ...
      error.value = response.error || 'Falha na geração de ontologia'
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = 'Falha na inicialização: ' + (err.message || 'Erro desconhecido')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || 'Falha ao carregar'
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = 'Falha ao carregar: ' + (err.message || 'Erro desconhecido')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = 'Tarefa de construção iniciada...'
      
      // Salvar task_id para polling
      const taskId = response.data.task_id
      
      // Polling de dados do grafo
      startGraphPolling()
      
      // Polling de status
      startPollingTask(taskId)
    } else {
      error.value = response.error || 'Falha ao iniciar construção'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'Falha ao iniciar construção: ' + (err.message || 'Erro desconhecido')
    buildProgress.value = null
  }
}

// Timer de polling do grafo
let graphPollTimer = null

// Iniciar polling do grafo
...
const fetchGraphData = async () => {
  try {
    // Obter informações do projeto para graph_id
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ Construção concluída, carregando dados...')
        ...
        buildProgress.value = {
          progress: 100,
          message: 'Construção concluída, carregando grafo...'
        }
        
        // Recarregar projeto para graph_id
        const projectResponse = await getProject(currentProjectId.value)
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Falha na construção: ' + (task.error || 'Erro desconhecido')
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  const nodes = nodesData.map(n => ({
    ...
    rawData: n // Dados originais
  }))
  
  // IDs para filtrar arestas válidas
