<template>
  ...
  >
    ...
    <div class="section-header">
      ...
      <span class="section-title">Registros de Simulação</span>
      <div class="section-line"></div>
    </div>
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- Cabeçalho: simulation_id e status -->
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                ...
                <div class="modal-empty" v-else>Sem arquivos associados</div>
              </div>
            </div>

            <!-- Divisória de reprodução -->
            <div class="modal-divider">
              <span class="divider-line"></span>
              <span class="divider-text">Reprodução da Simulação</span>
              <span class="divider-line"></span>
            </div>
            ...
            <div class="modal-actions">
              ...
              >
                ...
                <span class="btn-icon">◈</span>
                <span class="btn-text">Config. do Ambiente</span>
              </button>
              <button 
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Iniciar Simulação" e Step5 "Interação Profunda" precisam ser iniciados durante a execução e não suportam reprodução histórica</span>
            </div>
          </div>
        </div>
      </Transition>
...
const selectedProject = ref(null)  // Projeto selecionado (para o modal)
let observer = null
let isAnimating = false  // Bloqueio de animação
let expandDebounceTimer = null  // Timer de debounce
let pendingState = null  // Estado alvo pendente

// Config de layout dos cards
...
const containerStyle = computed(() => {
  ...
  // Estado expandido: altura dinâmica
  ...
  // Altura: linhas * altura + espaçamento
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    // Estado expandido: grade
    const transition = 'transform 700ms cubic-bezier(0.23, 1, 0.32, 1), opacity 700ms cubic-bezier(0.23, 1, 0.32, 1), box-shadow 0.3s ease, border-color 0.3s ease'

    const col = index % CARDS_PER_ROW
    const row = Math.floor(index / CARDS_PER_ROW)
    
    // Cards na linha, centralizar
...
// Classe por progresso de rodadas
...
// Navegar para construção do grafo
const goToProject = () => {
...
// Navegar para config. do ambiente
...
// Navegar para relatório de análise
const goToReport = () => {
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Atualizar estado alvo pendente
        ...
        // Se animando, registrar e processar depois
        if (isAnimating) return
        
        // Se estado igual, não processar
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Debounce para evitar cintilação
        // Expansão 50ms, recolhimento 200ms
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Verificar se estado pendente ainda é necessário
          ...
          // Desbloquear e verificar pendentes
          setTimeout(() => {
            isAnimating = false
            
            // Após animação, verificar pendentes
...
.status-icon:nth-child(2).available { color: #F59E0B; } /* Config. do Ambiente - laranja */
