    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Iniciando...' : 'Iniciar geração do relatório de resultados' }} 
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- DO_NOTHING: Nenhuma ação (silêncio) -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Máximo de rodadas do Step2
  minutesPerRound: {
    type: Number,
    default: 30 // Padrão 30 min/rodada
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const runStatus = ref({})
const allActions = ref([]) // Todas as ações (incremental)
const actionIds = ref(new Set()) // IDs para deduplicação
const scrollContainer = ref(null)

// Computed
// Exibir ações em ordem cronológica
...
// Resetar estados (para reiniciar)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
  ...
  stopPolling()  // Parar polling existente
}
...
const doStartSimulation = async () => {
  ...
  try {
    const params = {
      ...
      enable_graph_memory_update: true  // Habilitar atualização dinâmica
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`Máximo de rodadas: ${props.maxRounds}`)
    }
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Logs antigos limpos, reiniciando simulação')
      }
...
// Rastrear última rodada por plataforma
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detectar mudanças de rodada por plataforma
      ...
      // Detectar conclusão da simulação
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Verificação extra de conclusão
      // Detectar via status de conclusão
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Simulação de todas as plataformas concluída')
        }
    ...
    console.warn('Falha ao obter status:', err)
  }
}

// Verificar conclusão das plataformas
const checkPlatformsCompleted = (data) => {
  // Sem dados de plataforma
  if (!data) return false
  
  // Verificar status por plataforma
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // Se uma concluiu, verificar todas
  // Verificar habilitação via actions_count
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Adicionar novas ações (deduplicar)
      ...
      // Não rolar automaticamente
      // Novas ações no final
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Solicitação de relatório enviada, aguarde...')
    return
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Tarefa de relatório iniciada: ${reportId}`)
      
      // Ir para página do relatório
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ Falha no relatório: ${res.error || 'Erro desconhecido'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ Exceção no relatório: ${err.message}`)
    isGeneratingReport.value = false
  }
}
...
onMounted(() => {
  addLog('Step3 inicialização da simulação')
  if (props.simulationId) {
    doStartSimulation()
  }
})
