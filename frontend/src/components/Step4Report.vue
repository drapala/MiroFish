    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - exibir após conclusão -->
          <button v-if="isComplete" class="next-step-btn" @click="goToInteraction">
            <span>Entrar na Interação Profunda</span>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Conteúdo gerado (mas seção pode não estar completa) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Dica especial para resposta final -->
...
const toggleRawResult = (timestamp, event) => {
  // Posição do botão relativa ao viewport
  ...
  // Ajustar scroll após DOM update
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extrair número de entrevistados
    ...
    // Extrair motivos de seleção
    const reasonMatch = text.match(/### 采访对象选择理由\n([\s\S]*?)(?=\n---\n|\n### 采访实录)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Parsear motivo de cada pessoa
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Formato 1: n. **nome(index=X)**: motivo
        // Ex: 1. **alumni_345(index=1)**: como alumni...
        ...
        // Ex: - selecionar pai_601(index 0): como representante...
        if (!headerMatch) {
        ...
        // Ex: - **pai_601(index 0)**: como representante...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // Continuação do motivo
          currentReason.push(line.trim())
        }
      }
      
      // Salvar último motivo
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extrair respostas - Twitter e Reddit
      ...
      if (answerMatch) {
        ...
        // Fallback de plataforma (formato antigo)
        if (!twitterMatch && redditMatch) {
          // Apenas Reddit, copiar se não for placeholder
          if (interview.redditAnswer && interview.redditAnswer !== '(Sem resposta nesta plataforma)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(Sem resposta nesta plataforma)') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // Sem marca de plataforma (formato antigo)
          interview.twitterAnswer = answerText
        }
      }
      
      // Extrair citações-chave
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Cadeias de relação (${props.result.relations.length})`)
        ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Fatos-chave recentes na memória temporal'),
          ...
          }, expandedFacts.value ? `Recolher ▲` : `Expandir todos ${props.result.facts.length} ▼`)
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Cadeias de relação'),
          ...
          }, expandedRelations.value ? `Recolher ▲` : `Expandir todos ${props.result.relations.length} ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Subquestões de análise por drift query'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'Sem memórias-chave atuais'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'Sem entidades centrais'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'Sem cadeias de relação')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Memórias válidas (${props.result.activeFacts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Sem memórias válidas atuais'),
          props.result.activeFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedActive.value = !expandedActive.value }
          }, expandedActive.value ? `Recolher ▲` : `Expandir todos ${props.result.activeFacts.length} ▼`)
        ]),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Sem memórias históricas'),
          props.result.historicalFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedHistorical.value = !expandedHistorical.value }
          }, expandedHistorical.value ? `Recolher ▲` : `Expandir todos ${props.result.historicalFacts.length} ▼`)
        ]),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Sem entidades envolvidas'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Verificar placeholder de plataforma
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(Sem resposta nesta plataforma)' || t === '(Sem resposta nesta plataforma)' || t === '[Sem resposta]'
    }

    // Dividir resposta por número da questão
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "问题X：" ou "问题X:" (formato chinês do backend)
      // 2. "1. " ou "\n1. " (formato numérico)
      let matches = []
      let match
      ...
      // Fallback para formato numérico
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Se não encontrou, retornar completo
      if (matches.length <= 1) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Falha: primeira questão retorna completa
      return qIdx === 0 ? answer : ''
    }
    
    // Verificar respostas de duas plataformas
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Ambas com respostas reais e diferentes
...
// Etapa ativa atual (para exibição no topo)
const activeStep = computed(() => {
  const steps = workflowSteps.value
  // Encontrar etapa ativa
  const active = steps.find(s => s.status === 'active')
  if (active) return active
  
  // Se nenhuma ativa, retornar última concluída
...
const renderMarkdown = (content) => {
  ...
  // Envolver listas não ordenadas
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Envolver listas ordenadas
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Limpar espaços entre itens
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Limpar espaços após tag de abertura
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Limpar espaços antes da tag de fechamento
  ...
  // Limpar <br> antes/depois de blocos
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Limpar <p><br> antes de blocos
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Limpar <br> consecutivos
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Limpar <br> entre blocos e parágrafos
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Corrigir numeração de listas separadas
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Limpar estado de loading
            emit('update-status', 'completed')
            stopPolling()
            // Scroll unificado após loop
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Se concluído, rolar ao topo; senão seguir logs
            if (isComplete.value) {
...
// Extrair resposta final do LLM response
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Extrair conteúdo de <final_answer>
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Cinza escuro, não muda com status */
  font-weight: 500;
}
