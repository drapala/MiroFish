<template>
  <div class="graph-panel">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'Atualização em tempo real da memória de curto/longo prazo GraphRAG' : 'Atualizando em tempo real...' }}
        </div>
        
        <!-- Aviso após término da simulação -->
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Ainda há conteúdo sendo processado, recomenda-se atualizar o grafo manualmente depois</span>
        ...
        <!-- Painel de detalhes do nó/aresta -->
        <div v-if="selectedItem" class="detail-panel">
          ...
          <div v-else class="detail-content">
            <!-- Detalhes do grupo de auto-laço -->
    ...
    <!-- Botão de exibição de rótulos das arestas -->
...
const expandedSelfLoops = ref(new Set()) // Itens de auto-laço expandidos
...
// Observar mudança em isSimulating, detectar término da simulação
...
// Alternar estado expandido/recolhido dos itens de auto-laço
...
const renderGraph = () => {
  ...
  // Processar dados das arestas, calcular quantidade e índice de arestas entre o mesmo par de nós
  const edgePairCount = {}
  const selfLoopEdges = {} // Arestas de auto-laço agrupadas por nó
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Contar quantidade de arestas entre cada par de nós, coletar arestas de auto-laço
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      // Auto-laço - coletar em array
  ...
  // Registrar qual aresta do par de nós está sendo processada
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // Nós de auto-laço já processados
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Aresta de auto-laço - adicionar apenas uma aresta combinada por nó
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Armazenar detalhes de todas as arestas de auto-laço
        }
      })
      return
    }
    ...
    // Verificar se a direção da aresta coincide com a direção padronizada (UUID origem < UUID destino)
    ...
    if (totalCount > 1) {
      ...
      // Se a direção da aresta for oposta à direção padronizada, inverter curvatura
  ...
  // Simulation - ajustar dinamicamente a distância entre nós com base na quantidade de arestas
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Ajustar dinamicamente a distância com base na quantidade de arestas entre este par de nós
      // Distância base 150, aumento de 40 para cada aresta adicional
    ...
    // Adicionar gravidade central para agrupar nós isolados na área central
  ...
  const getLinkPath = (d) => {
    ...
    // Detectar auto-laço
    if (d.isSelfLoop) {
      // Auto-laço: desenhar um arco saindo do nó e retornando
      const loopRadius = 30
      // Sair do lado direito do nó, contornar e retornar
      const x1 = sx + 8  // Deslocamento do ponto inicial
      const y1 = sy - 4
      const x2 = sx + 8  // Deslocamento do ponto final
      const y2 = sy + 4
      // Usar arco para desenhar auto-laço (sweep-flag=1 sentido horário)
    ...
    // Calcular ponto de controle da curva - ajustar dinamicamente com base na quantidade de arestas e distância
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Deslocamento perpendicular à direção da conexão, calculado por proporção da distância
    // Quanto mais arestas, maior a proporção de deslocamento
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, aumento de 5% para cada aresta adicional
  ...
  const getLinkMidpoint = (d) => {
    ...
    // Detectar auto-laço
    if (d.isSelfLoop) {
      // Posição do rótulo de auto-laço: lado direito do nó
      return { x: sx + 70, y: sy }
    }
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // Destacar aresta atualmente selecionada
  ...
  // Link labels background (fundo branco torna o texto mais legível)
  const linkLabelBg = linkGroup.selectAll('rect')
    ...
    .on('click', (event, d) => {
      ...
      linkLabelBg.attr('fill', 'rgba(255,255,255,0.95)')
      linkLabels.attr('fill', '#666')
      // Destacar aresta correspondente
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Apenas registrar posição, não reiniciar simulação (distinguir clique e arraste)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detectar se realmente começou a arrastar (moveu além do limiar)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Reiniciar simulação apenas na primeira detecção de arraste real
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Deixar simulação parar gradualmente apenas se houve arraste real
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Atualizar posição dos rótulos das arestas (sem rotação, exibição horizontal mais legível)
  ...
  // Fechar painel de detalhes ao clicar no espaço vazio
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
...
// Observar botão de exibição dos rótulos das arestas
