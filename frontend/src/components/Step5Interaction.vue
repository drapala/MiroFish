        ...
        <div class="action-bar-header">
          ...
          <div class="action-bar-tabs">
            ...
            >
              ...
              <span>Enviar questionário ao mundo</span>
            </button>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">Versão de conversa rápida do agente de relatórios, com 4 ferramentas profissionais e memória completa do MiroFish</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InsightForge Análise Profunda</div>
                    <div class="tool-desc">Alinha dados de sementes reais com o estado do ambiente simulado, usando mecanismo Global/Local Memory para análise de atribuição profunda cross-temporal</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Algoritmo de travessia em largura baseado em grafo, reconstruindo caminhos de propagação de eventos e capturando a topologia completa do fluxo de informação</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Interface de consulta instantânea GraphRAG, com índice otimizado para extração rápida de atributos de nós e fatos discretos</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Entrevista autônoma, capaz de dialogar em paralelo com indivíduos do mundo simulado, coletando dados de opinião não estruturados e estados psicológicos</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="chatTarget === 'agent' && selectedAgent" class="agent-profile-card">
            ...
            <div v-if="showFullProfile && selectedAgent.bio" class="profile-card-body">
              <div class="profile-card-bio">
                <div class="profile-card-label">Descrição</div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Converse com o Report Agent para aprofundar o conteúdo do relatório' : 'Converse com indivíduos simulados para conhecer suas opiniões' }}
              </p>
            </div>
            <div 
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">Selecionar Pesquisados</span>
            ...
            <div class="setup-section">
              ...
              <textarea 
                ...
                class="survey-input"
                placeholder="Digite a pergunta para todos os selecionados..."
                rows="3"
              ></textarea>
            </div>
...
const chatHistoryCache = ref({}) // Cache de todos os históricos de conversa: { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
// Salvar histórico atual no cache
...
const selectReportAgentChat = () => {
  ...
  // Restaurar histórico do Report Agent
...
const selectAgent = (agent, idx) => {
  ...
  // Restaurar histórico do Agent
  chatHistory.value = chatHistoryCache.value[`agent_${idx}`] || []
  addLog(`Selecionar Interlocutor: ${agent.username}`)
}
...
const renderMarkdown = (content) => {
  ...
  // Limpar <br> entre blocos
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Corrigir numeração de listas
...
const sendMessage = async () => {
  ...
  try {
    ...
    addLog(`Falha ao enviar: ${err.message}`)
    chatHistory.value.push({
      role: 'assistant',
...
const sendToReportAgent = async (message) => {
  addLog(`Enviando ao Report Agent: ${message.substring(0, 50)}...`)
...
const sendToAgent = async (message) => {
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? 'Entrevistador' : 'Você'}：${msg.content}`)
      .join('\n')
    prompt = `Histórico da nossa conversa:\n${historyContext}\n\nMinha nova pergunta é: ${message}`
  }
  ...
  if (res.success && res.data) {
    // Caminho correto: res.data.result.results é um dicionário
    // Formato: {"twitter_0": {...}, "reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // Converter dicionário em array, priorizar Reddit
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // Priorizar Reddit, depois Twitter
...
const submitSurvey = async () => {
  ...
  addLog(`Enviando questionário para ${selectedAgents.value.size} pesquisados...`)
...
const loadAgentLogs = async () => {
  ...
  try {
    ...
    addLog(`Falha ao carregar logs: ${err.message}`)
  }
}
...
const loadProfiles = async () => {
  ...
  try {
    ...
    addLog(`Falha ao carregar indivíduos: ${err.message}`)
  }
}
...
/* Painel esquerdo - consistente com Step4Report.vue */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
...
/* Corrigir numeração - CSS counter para ols contínuos */
.message-text {
...
/* Estilo de citação no chat/questionário */
